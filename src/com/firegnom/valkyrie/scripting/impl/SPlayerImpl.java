...
public class SPlayerImpl implements SPlayer {
	 ...
	 * @param v the v
	 */
	public SPlayerImpl(final View v) {
		this.v = v;
	}
	 ...
	 */
	@Override
	public boolean canReach(final int x, final int y) {
	...
	public int distance(final int x, final int y) {
		final Player p = GameController.getInstance().user;
		final float dx = p.position.x - x;
		final float dy = p.position.y - y;
	 ...
	 * @return the int
	 */
	public int distance(final Position p) {
	 ...
	 */
	@Override
	public com.firegnom.valkyrie.net.protocol.Path getPath(final int x,
			final int y) {
	 ...
	 */
	@Override
	public void moveW(final int x, final int y) {
		move(x, y);
		waitForMove();
	}
